...
ManifestRule = Callable[[dict], list[Finding]]
...
def rule_missing_column_description(manifest: dict) -> list[Finding]:
    findings: list[Finding] = []
        ...
        for column_name, column in (node.get("columns") or {}).items():
            ...
            findings.append(
                Finding(
                    ...
                    message=f"Column `{column_name}` is missing a description in the manifest metadata.",
                    file_path=file_path,
                )
            )
...
def collect_manifest_findings(manifest: dict) -> list[Finding]:
    findings: list[Finding] = []
...
def build_review_result(
    ...
    for missing_target in missing_targets:
        findings.append(
            Finding(
                ...
                message="Manifest referenced a file that was not found in the uploaded project archive.",
    ...
    if findings:
        status = "warning" if any(finding.severity == "warning" for finding in findings) else "success"
